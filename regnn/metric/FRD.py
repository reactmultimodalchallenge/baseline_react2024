...
def compute_FRD_mp(data_dir, pred, em, val_test='val', p=4):
    # pred: N 10 750 25
    # speaker: N 750 25

    if val_test == 'val':
        neighbour_matrix = np.load(os.path.join(data_dir, 'neighbour_emotion_val.npy'))
    else:
        neighbour_matrix = np.load(os.path.join(data_dir, 'neighbour_emotion_test.npy'))
